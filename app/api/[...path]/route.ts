...
export async function GET(request: Request) {
  ...
  if (route === "audit/files") {
    ...
    try {
      ...
      const albumsByMedia=new Map<string,string>();
      for(const memory of memories){const mediaId=String(memory?.mediaId||"");const albumName=String(memory?.albumName||"").trim();if(mediaId&&albumName&&!albumsByMedia.has(mediaId))albumsByMedia.set(mediaId,albumName);}
      for(const row of rows){row.r2AlbumName=row.r2AlbumName||albumsByMedia.get(String(row.mediaId))||null;row.arweaveAlbumName=row.arweaveAlbumName||(row.arweaveId?row.r2AlbumName:null);}
      return json({rows});
    } catch(error) { console.error("Audit ledger query failed",error); return json({error:"Audit records could not be loaded from permanent storage."},500); }
  }
  ...
  if (route === "arweave/albums") {
    ...
    const snapshots=await query<any>("SELECT data FROM vault_snapshots WHERE user_id=$1",[user.id]);
    const memories=Array.isArray(snapshots[0]?.data?.memories)?snapshots[0].data.memories:[];
    const albumMemories=memories.filter((item:any)=>item?.albumName&&item?.mediaId);
    const mediaIds=[...new Set<string>(albumMemories.map((item:any)=>String(item.mediaId)))];
    const media=mediaIds.length?await query<any>("SELECT id,original_name AS name,content_type AS \"contentType\",size_bytes AS \"sizeBytes\",status FROM media_objects WHERE user_id=$1 AND deleted_at IS NULL AND id=ANY($2::text[])",[user.id,mediaIds]):[];
    const archives=mediaIds.length?await query<any>("SELECT DISTINCT ON (media_object_id) media_object_id AS \"mediaId\",id AS \"jobId\",status,transaction_id AS \"transactionId\",error_message AS error FROM arweave_storage_jobs WHERE user_id=$1 AND media_object_id=ANY($2::text[]) ORDER BY media_object_id,created_at DESC",[user.id,mediaIds]):[];
    const mediaById=new Map(media.map((item:any)=>[item.id,item]));
    const archiveByMedia=new Map(archives.map((item:any)=>[item.mediaId,item]));
    const groups=new Map<string,any[]>();
    for(const memory of albumMemories){
      const details:any=mediaById.get(String(memory.mediaId)); if(!details)continue;
      const archive:any=archiveByMedia.get(String(memory.mediaId));
      const item={memoryId:String(memory.id),mediaId:String(memory.mediaId),title:String(memory.title||details.name),name:details.name,contentType:details.contentType,sizeBytes:Number(details.sizeBytes||0),mediaStatus:details.status,archiveStatus:archive?.status||"r2_only",archiveJobId:archive?.jobId||null,transactionId:archive?.transactionId||null,archiveError:archive?.error||null};
      const name=String(memory.albumName).trim(); if(!name)continue;
      if(!groups.has(name))groups.set(name,[]); groups.get(name)!.push(item);
    }
    const albums=[...groups.entries()].map(([albumName,items])=>({albumName,itemCount:items.length,totalBytes:items.reduce((sum,item)=>sum+item.sizeBytes,0),confirmedCount:items.filter(item=>item.archiveStatus==="confirmed").length,pendingCount:items.filter(item=>["staging","queued","uploading","submitted"].includes(item.archiveStatus)).length,failedCount:items.filter(item=>item.archiveStatus==="failed").length,eligibleCount:items.filter(item=>item.archiveStatus==="r2_only"&&item.mediaStatus==="ready"&&item.sizeBytes>0&&item.sizeBytes<=100*1024*1024-32).length,ineligibleCount:items.filter(item=>item.archiveStatus==="r2_only"&&(item.mediaStatus!=="ready"||item.sizeBytes<=0||item.sizeBytes>100*1024*1024-32)).length,items})).sort((a,b)=>a.albumName.localeCompare(b.albumName));
  ...
  if (route === "arweave/archive/price") {
    const user=await authenticatedUser(request); if(!user)return json({error:"Authentication required."},401);
    const bytes=Math.max(1,Math.min(2*1024*1024*1024,Number(new URL(request.url).searchParams.get("bytes")||1)));
  ...
  if (route.startsWith("arweave/archive/receipt/")) {
    const user=await authenticatedUser(request); if(!user)return json({error:"Authentication required."},401);
    const id=route.replace("arweave/archive/receipt/","");
    const rows=await query<any>("SELECT id,media_object_id AS \"mediaId\",original_name AS name,original_content_type AS \"contentType\",encrypted_size_bytes AS \"encryptedBytes\",payload_sha256 AS \"payloadHash\",encryption_algorithm AS encryption,status,transaction_id AS \"transactionId\",block_height AS \"blockHeight\",confirmations,created_at AS \"createdAt\",submitted_at AS \"submittedAt\",confirmed_at AS \"confirmedAt\" FROM arweave_storage_jobs WHERE id=$1 AND user_id=$2",[id,user.id]);
    if(!rows[0])return json({error:"Archive receipt not found."},404);
    const receipt={schema:"aeterna-archive-receipt/v1",...rows[0],gatewayUrl:rows[0].transactionId?"https://arweave.net/"+rows[0].transactionId:null,generatedAt:new Date().toISOString()};
    return new Response(JSON.stringify(receipt,null,2),{headers:{"Content-Type":"application/json; charset=utf-8","Content-Disposition":`attachment; filename="aeterna-archive-${id}.json"`,"Cache-Control":"no-store"}});
  }
...
export async function POST(request: Request) {
  ...
  if (route === "arweave/archive/complete") {
    ...
    const queued=await query<any>("UPDATE arweave_storage_jobs SET status=$$queued$$,updated_at=NOW() WHERE id=$1 RETURNING payload_sha256 AS \"payloadHash\",encrypted_size_bytes AS \"encryptedBytes\",created_at AS \"createdAt\"",[id]); await notifyUser(user.id,"info","Permanent archive queued","Your encrypted archive is queued for Arweave.",{actionView:"audit",entityType:"arweave_storage_job",entityId:id}); return json({success:true,jobId:id,receipt:{jobId:id,payloadHash:queued[0].payloadHash,encryptedBytes:Number(queued[0].encryptedBytes),status:"queued",createdAt:queued[0].createdAt,receiptUrl:"/api/arweave/archive/receipt/"+id}});
  }
  if (route === "arweave/archive/link") {
    const user=await authenticatedUser(request); if(!user)return json({error:"Authentication required."},401);
    const jobId=String(body.jobId||""),mediaId=String(body.mediaId||""),albumName=String(body.albumName||"").trim().slice(0,200)||null;
    const rows=await query("UPDATE arweave_storage_jobs a SET media_object_id=m.id,album_name=COALESCE($1,a.album_name),updated_at=NOW() FROM media_objects m WHERE a.id=$2 AND a.user_id=$3 AND m.id=$4 AND m.user_id=$3 AND (a.media_object_id IS NULL OR a.media_object_id=m.id) RETURNING a.id",[albumName,jobId,user.id,mediaId]);
    return rows[0]?json({success:true,jobId,mediaId}):json({error:"Archive handoff or media item was not found."},404);
  }
