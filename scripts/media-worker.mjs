...
while (!stopping) {
  try {
    const response = await fetch(appUrl + "/api/internal/media-worker", {
      method: "POST",
      headers: { Authorization: "Bearer " + secret, "Content-Type": "application/json" },
      body: "{}",
    });
    if (!response.ok) console.error("Media worker request failed:", response.status, await response.text());
    const result = response.ok ? await response.json() : null;
    await wait(result?.busy ? 3000 : 1000);
  } catch (error) {
    console.error("Media worker connection failed:", error);
    await wait(5000);
  }
}
