...
export async function queuePhotosItems(userId: string, id: string) {
  ...
  for (const item of items.slice(0,500)) {
    ...
    await execute("INSERT INTO media_import_jobs (id,user_id,provider_connection_id,provider_file_id,provider_file_name,status,provider_payload) VALUES ($1,$2,$3,$4,$5,$$queued$$,$6::jsonb) ON CONFLICT (user_id,provider_connection_id,provider_file_id) DO UPDATE SET id=EXCLUDED.id,status=$$queued$$,provider_payload=EXCLUDED.provider_payload,bytes_transferred=0,progress=0,error_message=NULL", [jobId, userId, rows[0].provider_connection_id, "photos:" + item.id, name, JSON.stringify({ provider: "google-photos", baseUrl: file.baseUrl, mimeType: file.mimeType, type: item.type, createTime: item.createTime || null })]);
