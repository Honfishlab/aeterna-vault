...
async function uploadWithCheckpoints(download: Response, job: any, objectKey: string, mimeType: string, total: number) {
  ...
  if (!uploadId) {
    const created = await client.send(new CreateMultipartUploadCommand({ Bucket: r2Bucket(), Key: objectKey, ContentType: mimeType, Metadata: job.album_name ? { "album-name": String(job.album_name).slice(0, 200) } : undefined }));
...
export async function processNextImportJob() {
  ...
  try {
    ...
    if (existingObject?.status === "ready") {
      ...
      await execute("UPDATE media_objects SET width=COALESCE($1,width),height=COALESCE($2,height),duration_ms=COALESCE($3,duration_ms),captured_at=COALESCE($4,captured_at),thumbnail_object_key=COALESCE($5,thumbnail_object_key),album_name=COALESCE($6,album_name) WHERE id=$7", [payload.width, payload.height, payload.durationMillis, payload.createTime, thumbnailKey, job.album_name, existingObject.id]);
    ...
    if (existingObject) {
      await execute("UPDATE media_objects SET original_name=$1,content_type=$2,size_bytes=$3,width=$4,height=$5,duration_ms=$6,captured_at=$7,album_name=$8,status=$$pending$$,etag=NULL,completed_at=NULL WHERE id=$9", [job.provider_file_name, mimeType, total, payload.width, payload.height, payload.durationMillis, payload.createTime, job.album_name, mediaId]);
    } else {
      await execute("INSERT INTO media_objects (id,user_id,object_key,original_name,content_type,size_bytes,status,source_provider,source_id,width,height,duration_ms,captured_at,album_name) VALUES ($1,$2,$3,$4,$5,$6,$$pending$$,$7,$8,$9,$10,$11,$12,$13)", [mediaId, job.user_id, objectKey, job.provider_file_name, mimeType, total, payload.provider, job.provider_file_id, payload.width, payload.height, payload.durationMillis, payload.createTime, job.album_name]);
    }
