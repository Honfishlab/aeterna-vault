...
export function CloudImportModal({ isOpen, onClose, onImported }: {
  ...
  const importSelected = async () => {
    ...
    try {
      ...
      if (body.jobs?.length) window.dispatchEvent(new CustomEvent("aeterna-import-jobs", { detail: body.jobs }));
      onImported(body.imported || []);
      if (body.failed?.length) setError(`${body.failed.length} item(s) could not be imported.`);
      else if (!body.jobs?.length) onClose();
    } catch (reason: any) {
        ...
        <div className="p-5 flex-1 overflow-y-auto">
          ...
          {loading && !files.length ? (
            ...
            <>
              ...
              <BackgroundImportProgress onImported={onImported} />
            </>
          )}
        </div>
