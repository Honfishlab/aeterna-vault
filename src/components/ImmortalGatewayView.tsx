...
export const ImmortalGatewayView: React.FC<ImmortalGatewayViewProps> = () => {
  const [activePanel, setActivePanel] = useState<"overview" | "security" | "publish" | "records">("overview");
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      <section className="cosmic-card-gold rounded-3xl p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#F5D77F]">Vault protection and permanent storage</p><h1 className="mt-2 font-cinzel text-3xl font-bold text-[#FFF2A8]">Vault Security</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[#D8CCE8]">Protect access to your permanent files, keep a recovery method, and choose what—if anything—you want to publish publicly. Your everyday memories remain private unless you deliberately archive or publish them.</p></div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => void load()} disabled={loading} className="gold-beveled-btn flex min-h-11 items-center gap-2 px-4 py-2 text-xs disabled:opacity-40"><RefreshCw className={loading ? "h-4 w-4 animate-spin" : "h-4 w-4"}/>Check status</button>
          </div>
        </div>
      ...
      {activePanel==="overview"&&<>
        <section className="grid gap-4 md:grid-cols-[1.25fr_1fr]">
          <div className="cosmic-card-gold rounded-3xl p-6"><div className="flex items-center gap-3"><div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${configured?"bg-emerald-500/15 text-emerald-300":"bg-amber-500/15 text-amber-200"}`}><ShieldCheck className="h-6 w-6"/></div><div><p className="text-xs font-semibold text-[#C8B1E4]">Overall status</p><h2 className="font-cinzel text-2xl text-[#FFF2A8]">{configured?"Archive service is ready":"Archive setup needs attention"}</h2></div></div><p className="mt-4 text-sm leading-6 text-[#D8CCE8]">{configured?`${confirmedArchiveCount} permanent ${confirmedArchiveCount===1?"copy is":"copies are"} confirmed. Review access and recovery before publishing more.`:"Permanent archiving is unavailable until the service wallet is configured. Your private vault files are not affected."}</p></div>
      ...
      <section className="rounded-3xl border border-[#DFB260]/30 bg-[#160D27]/92 p-6"><p className="text-xs font-semibold uppercase tracking-widest text-[#F5D77F]">Access and recovery</p><h2 className="mt-2 font-cinzel text-2xl text-[#FFF2A8]">Two things protect your permanent files</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#120B21] p-4"><strong className="text-[#FFF2A8]">1. Master passphrase</strong><p className="mt-2 text-sm leading-5 text-[#C8B1E4]">Used in this browser to unlock encrypted permanent copies. It is never sent to Aeterna.</p></div><div className="rounded-2xl bg-[#120B21] p-4"><strong className="text-[#FFF2A8]">2. Recovery key</strong><p className="mt-2 text-sm leading-5 text-[#C8B1E4]">A separate key you keep somewhere safe. You need it if you forget the master passphrase.</p></div></div><p className="mt-4 rounded-xl border border-amber-300/25 bg-amber-500/10 p-3 text-xs leading-5 text-amber-100"><strong>Important:</strong> Aeterna cannot reset either secret or decrypt your permanent files. Store the recovery key outside this vault.</p></section>
      ...
      {activePanel==="records"&&<section className="rounded-3xl border border-[#DFB260]/30 bg-[#160D27]/92 p-5"><p className="text-xs font-semibold uppercase tracking-widest text-[#F5D77F]">Permanent copies</p><h2 className="mt-1 font-cinzel text-2xl text-[#FFF2A8]">Verify and recover archived files</h2><p className="mt-2 text-sm text-[#C8B1E4]">Select a file to check that its encrypted copy is still available. Technical proof details are shown only for the selected record.</p></section>}
      {activePanel==="records"&&(!loading && jobs.length === 0 ? <section className="cosmic-card rounded-2xl p-10 text-center"><Lock className="mx-auto h-8 w-8 text-[#F5D77F]"/><h2 className="mt-3 font-cinzel text-xl text-[#FFF2A8]">No permanent copies yet</h2><p className="mt-2 text-sm text-[#C8B1E4]">Your private vault is still available. Permanent copies appear here only after you deliberately archive a file.</p></section> :
