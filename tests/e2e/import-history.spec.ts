...
test.beforeEach(async ({ page }) => {
  await page.route("**/api/audit/files", route => route.fulfill({
    ...
    body: JSON.stringify({ rows: [
      { id:"done",name:"family.mp4",mediaId:"media-done",archiveJobId:"archive-done",hasThumbnail:true,r2AlbumName:"Family Reunion",arweaveAlbumName:"Family Reunion",contentType:"video/mp4",status:"complete",processingStatus:"ready",archiveStatus:"confirmed",arweaveId:"abcdefghijklmnopqrstuvwxyz1234567890ABCDEFG",bytesTotal:18874368,r2UploadedAt:"2026-07-27T00:05:00Z",permanentArchiveDate:"2026-07-27T00:10:00Z" },
...
test("shows the import audit as a plain file ledger", async ({ page }) => {
  ...
  for (const heading of ["Preview","File","R2 album","Arweave album","Type","Size","Status","Receipt","Arweave ID","Upload to R2 date","Permanent archive date"]) {
  ...
  await expect(permanentRow.getByRole("button",{name:"Download receipt"})).toBeVisible();
